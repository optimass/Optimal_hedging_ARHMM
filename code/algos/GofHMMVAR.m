function out = GofHMMVAR(R,reg,max_iter, prec,n_sample)
%
% Goodness-of-fit test for multivariate Autoregressive Gaussian HMM models 
...
%          PHI: (p x p x reg)  PHI(:,:,;) is the autoregressive coefficent
%                                       of regime k
...
[mu,A,phi,Q,nu,eta,cvm_est] = EstHMMVAR_mex(R,reg,max_iter, prec);

fprintf('\n End of estimation\n'); 

%% Parametric bootstrap

cvm_sim = zeros(n_sample,1);            n = size(R,1);
...
parfor i = 1:n_sample
    ...
    try
        R1 = SimHMMVAR(mu,A,phi,Q,eta0(i),n);
        [~,~,~,~,~,~,cvm_sim(i)] = EstHMMVAR_mex(R1,reg,max_iter, prec);
    catch
        R1 = SimHMMVAR(mu,A,phi,Q,eta0(i),n);
        [~,~,~,~,~,~,cvm_sim(i)] = EstHMMVAR_mex(R1,reg,max_iter, prec);
    end
