function [S,C,a,c,rho,phi1] = HedgingLI_mass(mu,vol,T,K,r,n,put,minS,maxS,S0,m)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     I0 =  zeros((m+1),(m+1));   % les I sont necessaire pour calculer les espérances
     I1 =  I0;
     I2 =  I0;
 
     mu2 = mu*Tp -rp;  %periodic mean of excess returns
     sigma2 = volp^2;
     c1 = exp(mu2+0.5*sigma2);   % cste utile souvent
     c2 = exp(2.0*(mu2+sigma2)); % cste utile souvent 
     M1 = c1-1.0;          % b
     M2 = c2+1.0 -2.0*c1;  % a
     ...
     ind2 =(2:(m+1))'; % all indice except first ( going to be usefull to fill the Is)
       
 %% Equations for A and B at q=0 and q=m 
    % on commence par calculer les I  sur le bord de la grille
    % ce ne sont pas des intervalle! ils sont sur les bords

           s  = S1(1); % prix le plus bas
           x  = log(s./S1);
           d0 = (x-mu2)/volp;
           d1 = d0 - volp;
           d2 = d1 - volp;
...
%%
 ...
 %% on calcule les autre
  ...
  %% 
   ...
   if put
       C(n+1,:)  = max(Kp-S',0);   % put
   end


  C(1:n,1) = C(n+1,1);   % coté du prix le plus bas
  a(:,1) = C(1:n,1)*rho; % dans le pdf de Bruno
...
for k = n:-1:1
    ...
    %%   A and B 
     A(k,1:m) = C(k+1,1:m); % temp
     B(k,1:m) = (C(k+1,2:(m+1))-A(k,1:m)) ./ (S(2:m+1)-S(1:m))';  % ok
                   
     A(k,1:m) = A(k,1:m)-B(k,1:m).*S(1:m)'; % ok
       
     A(k,m+1) = A(k,m); %  (parce que on ne peut plus calculer la "derivé" sur la bordure)
     B(k,m+1) = B(k,m); % ok     
...
%%    C and a
         ...
         for i = 2:(m+1)
           ...
           sumC = A(k,:)*( (1.0+rho)*I0(:,i)-rho*I1(:,i)) + ...
                        s*B(k,:)*( (1.0+rho)*I1(:,i) - ...
                            rho*I2(:,i));
                    
           %sum0 = A(k,:)*(1.0+rho)*I0(:,i);
           %sum1 = s*B(k,:)*(1.0+rho)*I1(:,i) - A(k,:)*rho*I1(:,i);
           %sum2 = -s*B(k,:)* rho*I2(:,i);
           
           %sumCtest = sum0 + sum1 + sum2;
